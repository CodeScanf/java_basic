            ...
            switch (instruction.getOpcode()) {
                case getstatic -> {
                    ...
                    if (className.contains("java")) {
                        ...
                        staticFiled = declaredField.get(null);
                        stack.peek().pushObjectToOperandStack(staticFiled);
                    }
                }
                case iconst_1 -> {
                ...
                case iconst_5 -> {
                    stack.peek().pushObjectToOperandStack(5);
                }
                case iload_0 -> {
                ...
                case invokevirtual -> {
                    ...
                    MethodInfo finalMethodInfo = classFile.getMethods(methodName).get(0);//不支持重载
                    Object[] args = new Object[params.size() + 1];//还有一个this本体所以需要 size + 1
                    for (int index = args.length - 1; index >= 0; index--) {
                        args[index] = stack.peek().operandStack.pop();
                    }
                ...
                case invokestatic -> {
                    ...
                    ClassFile classFile = classLoader.loadClass(className);
                    MethodInfo finalMethodInfo = classFile.getMethods(methodName).get(0);//不支持重载
                    Object[] args = new Object[params.size()];//静态函数不需要多一个
                    for (int index = args.length - 1; index >= 0; index--) {
                        args[index] = stack.peek().operandStack.pop();
                    }
                    StackFrame stackFrame = new StackFrame(finalMethodInfo, classFile.getConstantPool(), args);
